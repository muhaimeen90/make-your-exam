  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (data.results) {
        ...
        console.log("Set searchResults to:", data.results);
      }
    } catch (error) {
    ...
    const response = await fetch('/api/generate-pdf', {
      ...
      body: JSON.stringify({
        selections: sortedSelections.map(s => ({
          ...
          page_number: parseInt(s.page_number),
          crop_box: (s.crop_box && typeof s.crop_box.x === 'number') ? [
            parseFloat(s.crop_box.x),
            parseFloat(s.crop_box.y),
            parseFloat(s.crop_box.width),
            parseFloat(s.crop_box.height)
          ] : null
        }))
      })
    });
